...
typedef struct{
// Functions for the user application to access EIP variables

    UINT16 underTemp; // display value
    UINT16 minFlow; //min flow
    UINT16 flowOkay;//flow ok
    UINT16 leakRate;//leak rate
    UINT16 overTemp;//overtemp
    UINT8  responseTime;//response time
    UINT8  unit;//units
    UINT16 webStatus;//web status
    UINT16 firmwareRev;//firmware Rev
    UINT32 serialNumber;//serial Number
    UINT16 restartTime;//Restart Time
    UINT16 webSupply;//Website Supply
    UINT16 webReturn;//website Return
    UINT16 EIPStatus;//EIP Status Word
    UINT16 EIPCommand;//EIP Command Word
    UINT8  IPAddress[4];//IPAddressLocal
    UINT8  SMask[4];//SMaskLocal
    UINT16 venturiTime;//Extra
}__WebValues__;
...
void setWebsiteData(UINT8 Units,UINT16 MinFlow,UINT16 FlowOK,UINT16 LeakRate,UINT16 OverTemp,UINT8 ResponseTime,UINT16 RestartTime, UINT16 VNTime,UINT16 FirmwareRev, UINT32 SerialNum, UINT16 WebStatus, UINT16 WebSupply,UINT16 WebReturn);
